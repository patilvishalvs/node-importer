...
class NodeImporter {
    ...
    if (!empty($data)) {
      ...
      foreach ($fields_mapper as $key => $field) {
        ...
        switch ($settings_count) {
          ...
          case 2:
            if($target_type == 'file'){
              $file_content = file_get_contents($data[$key]);
              $file = file_save_data($file_content, 'public://file-'.REQUEST_TIME.'.png', FILE_EXISTS_REPLACE);
              $values[$field_name] = $file->id();
            }
            break;
          case 3:
            $values[$field_name] = NodeImporter::getTargetId($target_type, $target_bundle, $data[$key], $field['separator']);
            break;
        }
      }
  ...
  private static function getTargetId($target_type, $target_bundle, $value, $separator = ','){
